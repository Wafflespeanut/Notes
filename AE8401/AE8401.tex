...
\begin{itemize}
\item For a point load $P$ acting at the tip of the beam,
\end{itemize}
\definecolor{ffqqqq}{rgb}{1,0,0}
\definecolor{qqqqcc}{rgb}{0,0,0.8}
\definecolor{qqqqff}{rgb}{0,0,1}
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\hspace*{1cm}\raisebox{1.2pt}
\clip(-3.72,-3.18) rectangle (19.18,8.52);
...
\draw [line width=1.2pt] (1.04,2)-- (11.04,2);
\draw (1.01,1.89)-- (0.81,1.57);
\draw [->,color=qqqqcc] (11.01,3.72) -- (11.03,2.18);
\draw (10.78,4.7) node[anchor=north west] {\LARGE $P$};
\draw (0.1,2.62) node[anchor=north west] {\LARGE $\mathrm A$};
\draw (11.46,2.72) node[anchor=north west] {\LARGE $\mathrm B$};
\draw [->,color=ffqqqq] (0.46,-0.37) -- (0.44,1.17);
\draw (-0.24,-0.6) node[anchor=north west] {\LARGE $V_A$};
\draw (-1.75,3.3) node[anchor=north west] {\LARGE $M_C$};
\draw [shift={(-0.18,1.83)},line width=1.2pt,color=ffqqqq] plot[domain=1.52:4.76,variable=\t]({1*0.75*cos(\t r)+0*0.75*sin(\t r)},{0*0.75*cos(\t r)+1*0.75*sin(\t r)});
\draw [->,line width=1.2pt,color=ffqqqq] (-0.14,1.1) -- (0.11,1.1);
\draw [->,dash pattern=on 2pt off 2pt] (5.92,0.5) -- (11.1,0.5);
\draw [->,dash pattern=on 2pt off 2pt] (5.92,0.5) -- (1,0.5);
\draw (5.7,1.34) node[anchor=north west] {\LARGE $l$};
\begin{scriptsize}
\draw [color=qqqqff] (11.05,2)-- ++(-3.0pt,-3.0pt) -- ++(6.0pt,6.0pt) ++(-6.0pt,0) -- ++(6.0pt,-6.0pt);
\end{scriptsize}
\end{tikzpicture}
\\
\begin{enumerate}[(i)]
\item Maximum Bending Moment: {\LARGE $$M_C=Pl$$}
\item Slope at B: {\LARGE $$\theta_B=-\frac{Pl^2}{2EI}$$}
\item Maximum deflection:
{\LARGE $$\delta_C=-\frac{Pl^3}{3EI}{}$$}
\end{enumerate}
\newpage
...
$\ $
{\LARGE $$\tau_{\mathrm{max}_1}=\frac{\sigma_1-\sigma_2}{2}$$}
\end{document}