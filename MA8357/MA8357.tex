...
\draw [decorate,decoration={brace,amplitude=12pt,mirror,raise=4pt},thick,yshift=0pt]
...
\end{enumerate}
\newpage
\textbf{\section{\huge Fourier Transform}}
$\ $
\subsection{\LARGE Fourier Integral Theorem:}
{\LARGE $$f(x)=\frac 1{\pi}\int_0^{\infty}\bigg[\int_{-\infty}^{\infty}f(t)\ \mathrm{cos}\ s(t-x)\cdot dt\bigg]\cdot ds$$}
\\
\begin{itemize}
\item \textbf{Cosine Integral:} (if $f(x)$ is even)
{\LARGE $$f(x)=\frac 2{\pi}\int_0^{\infty}\bigg[\int_0^{\infty}f(t)\ \mathrm{cos}\ st\cdot dt\bigg]\mathrm{cos}\ sx\cdot ds$$}
\item \textbf{Sine Integral:} (if $f(x)$ is odd)
{\LARGE $$f(x)=\frac 2{\pi}\int_0^{\infty}\bigg[\int_0^{\infty}f(t)\ \mathrm{sin}\ st\cdot dt\bigg]\mathrm{sin}\ sx\cdot ds$$}
\end{itemize}
$\ $
\subsection{\LARGE Fourier Transform Pair:}
{\LARGE $$F(s)=\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}f(x)\ e^{isx}\cdot dx$$
\\
$$f(x)=\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}F(s)\ e^{-isx}\cdot ds$$}
\newpage
\textbf{\section{\huge Z-Transform}}
$\ $
\subsection{\LARGE Transformation rule:}
{\LARGE $$\textbf{Z}\{u_n\}=U(\textbf{z})=\sum_{n=0}^\infty u_n\  \textbf{z}^{-n}$$}
...
\end{enumerate}
\newpage
\subsection{\LARGE List of Transformations:}
\begin{enumerate}[(i)]
{\LARGE \item $$ \textbf{Z}(a^n)=\frac{\textbf{z}}{\textbf{z}-a}$$
\item $$\textbf{Z}(a^{n-1})=\frac{1}{\textbf{z}-a}$$
\item $$ \textbf{Z}(n)=\frac{\textbf{z}}{(\textbf{z}-1)^2}$$
\item $$ \textbf{Z}(n\ a^n)=\frac{a\textbf{z}}{(\textbf{z}-a)^2}$$
\item $$\textbf{Z}(n\ a^{n-1})=\frac{\textbf{z}}{(\textbf{z}-a)^2}$$
\item $$\textbf{Z}(n(n-1))=\frac{2\ \textbf{z}}{(\textbf{z}-1)^3}$$
\item $$\textbf{Z}(a^n \mathrm{cos\bigg(\frac{n\pi}{2}\bigg)})=\frac{\textbf{z}^2}{\textbf{z}^2+a^2}$$
\item $$\textbf{Z}(a^n \mathrm{sin\bigg(\frac{n\pi}{2}\bigg)})=\frac{a\textbf{z}}{\textbf{z}^2+a^2}$$
\item $$\textbf{Z}((n-1)\ a^{n-2})=\frac{1}{(\textbf{z}-a)^2}$$
\item $$\textbf{Z}((n-1)(n-2)\ a^{n-3})=\frac{2}{(\textbf{z}-a)^3}$$
\item $$\textbf{Z}((n+1)\ a^n)=\frac{\textbf{z}}{(\textbf{z}-a)^2}$$
}
\end{enumerate}
\end{document}